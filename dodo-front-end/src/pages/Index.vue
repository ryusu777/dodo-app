<template>
  <q-page class="row items-center justify-evenly">
    <div>
      <base-button label="View" @click="showView = true" />
      <base-button label="Add" @click="showAdd = true" />
      <base-button label="Confirm" @click="confirm" />
      <base-button label="Penjualan" @click="sendTransactionHeader()" />
    </div>

    <BaseDialog class="row justify-center" v-model="showView" title="View">
      <BaseField label="id" stack-label class="col-11"> asdaa </BaseField>

      <BaseField label="id" stack-label class="col-11"> asdaa </BaseField>
    </BaseDialog>

    <BaseAddDialog class="row justify-center" v-model="showAdd" title="Add">
      <base-input>Tambah</base-input>
      <base-date>Tanggal</base-date>
      <base-select-input :option="options" v-model="multiple">
        Pilihan
      </base-select-input>
      <base-check-box label="True"> </base-check-box>
    </BaseAddDialog>
  </q-page>
</template>
  ...
  setup() {
    ...
    async function sendTransactionHeader(): Promise<void> {
      try {
        ...
        await $router.push(`/selling-goods/${response.data.id || 0}`);
      } catch (err) {
        notifyError?.(err);
      }
    }
